...
def draw_window(red_ships, yellow_ships, red_bullets, yellow_bullets, red_missiles, yellow_missiles, yellow_stations, WIN, HUD, SPACE, DUST, FONT, spaceship_height, spaceship_width, explosion_group, global_state, fps, HEIGHT, WIDTH):
    ...
    for yellow in yellow_ships:
        if yellow.health > 0 and (yellow.cx - global_state.x > WIDTH or yellow.cx - global_state.x + yellow.width > 0) and (yellow.cy - global_state.y > HEIGHT or yellow.cy - global_state.y + yellow.height > 0):
            ...
            if not yellow.is_player and global_state.show_bars:  # ship is not player controlled
                ...
                yellow.x + 50 * yellow.health / yellow.Health - global_state.x, yellow.y - global_state.y - 5), bar_thickness)

                """ENERGY BAR"""
                pygame.draw.line(WIN, BLUE, (yellow.x - global_state.x, yellow.y - bar_thickness - global_state.y - 5), (yellow.x + 50 * yellow.energy / yellow.Energy - global_state.x, yellow.y - bar_thickness - global_state.y - 5), bar_thickness)
                ...
                health_text = FONT.render(f"{fps} Shields: {100 * yellow.health / yellow.Health:0.0f}%", 1, YELLOW)
                ...
                if yellow.target is not None and yellow.target.health > 0:
                    ...
                    SHIP = pygame.transform.rotate(yellow.target.image, yellow.target.angle)
    ...
    if global_state.menu is None:
        ...
        global_state.menu.draw_menu(FONT, HUD, yellow_ships, red_ships, yellow_stations, global_state)
