        ...
        if self.collidelist(target_list) != -1:  # bullet hits red
            ...
            for i in dmgList:
                ...
                if target_list[i].health <= 0:
                    explosion = ShipExplosion(target_list[i].centerx - gs.x, target_list[i].centery - gs.y)
                    explosion_group.add(explosion)
            bullet_list.remove(self)

            # pygame.event.post(pygame.event.Event(RED_HIT))
        elif self.timer > 600:  # missile runs out of thrust
            bullet_list.remove(self)
...
class Missile(pygame.Rect):
    ...
    def scoot(self, missile_list, target_list, explosion_group, gs):
        ...
        if self.target is None:
            explosion = MissileExplosion(self.centerx - gs.x, self.centery - gs.y, self.er)
            explosion_group.add(explosion)
