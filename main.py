...
FONT = pygame.font.SysFont('comicsans', 40)  # font type: not currently used
...
def main():
    ...
    if PCS == 'y':
        yellow = Ship(player_control, rnd.randint(0, 100), rnd.randint(0, HEIGHT), 0, 'yellow', 'Sprinter', 'PA', 'HE', True)
        yellow_ships.append(yellow)

    for i in range(nA):
        yellow = Ship(npc_control, rnd.randint(0, 100), rnd.randint(0, HEIGHT), rnd.randint(0, 359), 'yellow', 'Fighter', 'HV', 'HE')
        yellow_ships.append(yellow)

    for i in range(nE):
        red = Ship(npc_control, rnd.randint(10000, 10100), rnd.randint(0, HEIGHT), rnd.randint(0, 359), 'red', 'Fighter', 'HV', 'HE')
    ...
    while run:  # main loop
        ...
        """Render Window"""
        draw_window(red_ships, yellow_ships, red_bullets, yellow_bullets, red_missiles, yellow_missiles, WIN, SPACE, DUST, FONT, spaceship_height, spaceship_width, explosion_group, MyGS)
