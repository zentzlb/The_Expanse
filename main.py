...
def main():
    ...
    if PCS == 'y':
        yellow = Ship(player_control, rnd.randint(0, 200), rnd.randint(0, 1000), 0, 'yellow', 'Frigate', 'PA', 'torpedo', True)
    ...
    for i in range(nR):
        roid = Asteroid(rnd.randint(0, 10000), rnd.randint(0, 10000), rnd.randint(0, 359))
    ...
    while run:  # main loop
        ...
        for yellow in yellow_ships:
            if yellow.health > 0:
                yellow.scoot(yellow_bullets, yellow_missiles, red_ships, MyGS, yellow_stations, yellow_ships, asteroids)  # move ships
            else:
                yellow_ships.remove(yellow)  # remove dead ships

        for red in red_ships:
            if red.health > 0:
                red.scoot(red_bullets, red_missiles, yellow_ships, MyGS, [], [], asteroids)  # move ships
            else:
        ...
        """Render Window"""
        # t1 = time.time()
        draw_window(red_ships, yellow_ships, red_bullets, yellow_bullets, red_missiles, yellow_missiles, yellow_stations, asteroids, WIN, HUD, SPACE, DUST, spaceship_height, spaceship_width, explosion_group, MyGS, fps, HEIGHT, WIDTH)
        # t2 = time.time()
        # print(t2 - t1)
