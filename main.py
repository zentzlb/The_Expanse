...
def main():
    ...
    if PCS == 'y':
        yellow = Ship(player_control, 100, HEIGHT // 2, VEL, AV, ACC, 0, spaceship_height, spaceship_width, 'HV', 'HE', True)
    ...
    while run:  # main loop
        ...
        for yellow in yellow_ships:
            if yellow.health > 0:
                yellow.scoot(WIDTH, HEIGHT, yellow_bullets, yellow_missiles, red_ships, MyGS)
            else:
                yellow_ships.remove(yellow)

        for red in red_ships:
            if red.health > 0:
                red.scoot(WIDTH, HEIGHT, red_bullets, red_missiles, yellow_ships, MyGS)
            else:
        ...
        """Render Window"""
        draw_window(red_ships, yellow_ships, red_bullets, yellow_bullets, red_missiles, yellow_missiles, WIN, SPACE, FONT, spaceship_height, spaceship_width, explosion_group, MyGS)
